% =============================================================================
% Results section for the H2 paper.
...
We evaluate H2 on two classes of pretrained models, namely image
...
in the large-fan-in regime where H2 is expected to perform well.
...
per-channel symmetric uniform quantization. H2 is the proposed method
...
architectures with large fan-in. On ResNet-50, H2 outperforms
...
compression. On ViT-B/16 and ConvNeXt-Tiny, both H2 and QuaRot-RTN
reach essentially lossless accuracy at 4-bit, within 0.1 percentage
points of FP32. On EfficientNet-B0, H2 recovers 51.3 percentage points
...
H2 underperforms QuaRot-RTN at 4-bit, a result we analyze in
Section~\ref{sec:experiments-boundary}.
...
\hline
Model & Compression & FP32 & Baseline & QuaRot & H2 (ours) \\
\hline
...
below two percent. The striking exception is ViT-B/16, where H2
...
gap in favor of H2 and is the largest head-to-head margin in the
entire study. ConvNeXt-Tiny shows a smaller but still sizeable
advantage, with H2 at 27.0 percent versus QuaRot-RTN at 0.08 percent.
ResNet-18 also shows a positive H2 result at 4.6 percent, while on
...
fail catastrophically, with the notable exception of H2 on large-fan-
in architectures.}
\label{tab:vision-2bit}
\begin{tabular}{lrrrr}
\hline
Model & Compression & Baseline & QuaRot & H2 (ours) \\
\hline
...
Model & Baseline & QuaRot & H2 & Baseline & QuaRot & H2 \\
\hline
...
\hline
Model & Bits & Compression & Baseline & QuaRot & H2 (ours) \\
\hline
...
H2 reaches perplexity 8.68 compared to 10.67 for QuaRot-RTN and 10.98
for RTN-absmax. In relative terms, H2 increases perplexity by 8.9
...
the FP16 reference of 7.97, but H2 degrades far less than the
...
pattern where H2 is the only training-free method that produces
...
study. H2 reaches perplexity 77.03 compared to 74,433 for QuaRot-RTN
and 354,350 for RTN-absmax. This is a ratio of approximately 1,000
times for QuaRot and 4,600 times for RTN, and places H2 in a regime
...
winning configuration for H2 across all of our LLM experiments. H2
...
26.92, a 23.4 percent increase. H2 therefore recovers more than half
...
methods converge to within 0.7 percent of FP16 on Phi-1.5, with H2
retaining a small lead at 6-bit (21.87 for H2 versus 21.92 for
...
compression is mild. At 4-bit, H2 wins by a margin that depends on
the architecture. At 2-bit, H2 is the only viable method on large-
...
The practical rule from these observations is that H2 should be
...
the quantization error budget. Our results show H2 winning on
...
ResNet-50, ViT-B/16, ConvNeXt-Tiny, TinyLlama, and Phi-1.5, H2
...
H2 loses its theoretical advantage and underperforms QuaRot-RTN. This
...
that motivates H2 on ViT-B/16, where hidden dimension 768 satisfies the
