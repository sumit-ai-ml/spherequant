...
Three figures:
  ...
  fig_acc_vs_bits.png
    ...
    This is the headline comparison: does rotation (H2) or rotated-basis
  ...
  fig_weight_histograms.png
    Density of weight coordinates for each layer under three views:
      ...
      - H2:       baseline weight after post-hoc rotation, normalized rows
...
from rotation_utils import make_rotation, apply_rotation, SRHTRotation
...
def fig_weight_histograms(seed: int = 0):
    ...
    for i, ((name_b, Wb), (name_h, Uh)) in enumerate(zip(layers_base, layers_h3)):
        ...
        # H2: rotate baseline then normalize
...
def fig_ks_vs_layer(rows):
    ...
    Baseline/H2 use CNN3's nn.Sequential names (features.0/3/6, classifier);
