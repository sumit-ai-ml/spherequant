% =============================================================================
% Methodology section for the SphereQuant paper.
...
We describe SphereQuant, a post-training weight quantization pipeline that combines
...
ViT-B/16. This section derives SphereQuant from first principles, states each
...
covers every layer to which we apply SphereQuant in the experiments.
\end{remark}
...
\subsection{The SphereQuant quantization algorithm}
...
\begin{algorithm}[t]
\caption{SphereQuant post-training weight quantization, applied per layer.}
\label{alg:spherequant}
\begin{algorithmic}[1]
...
Algorithm~\ref{alg:spherequant}. The forward rotation in matrix form was
...
Our experiments compare SphereQuant against three baselines, each of which is also
...
orthogonal matrix $M$ as in SphereQuant and apply the forward rotation
$U = \widetilde{W}\, M^\top$. For SphereQuant the rotation is applied after row
...
and codebook design and can be applied on top of either QuaRot-RTN or SphereQuant.
...
omits the rotation. This is Algorithm~\ref{alg:spherequant} with step 3 replaced by
...
$N\, d\, b / 8$ bytes. The per-row norms for SphereQuant and per-row scales for
...
completes. For each such module we apply Algorithm~\ref{alg:spherequant} to
...
\subsection{Why SphereQuant beats uniform after rotation}
\label{sec:method-whybeats}

QuaRot-RTN and SphereQuant apply the same rotation and both use per-row scale
...
scale. Under SphereQuant, Lloyd--Max places the four centroids at the conditional
...
\end{equation}
so Bennett's formula yields $\mathcal{D}^\star_K = O(K^{-2}/d)$ for SphereQuant.
...
study where SphereQuant fails, namely MobileNet-V2. Depthwise convolutions with
...
carry enough of the quantization budget that SphereQuant still provides an overall
benefit, even though the depthwise layers are in the same flat-density
regime as MobileNet-V2.

A second mechanism, distinct from the failure of Beta concentration,
makes SphereQuant lose ground on small-fan-in layers. SphereQuant divides
each row by its L2 norm $\lVert w \rVert_2$, while RTN-absmax and
QuaRot-RTN divide by the maximum absolute value $\lVert w \rVert_\infty$.
For any row, $\lVert w \rVert_\infty \in [\lVert w \rVert_2 / \sqrt{d},\,
\lVert w \rVert_2]$, with the upper bound attained when a single
coordinate holds all of the row's energy. On rows whose energy is
concentrated in one or two outliers, which is the typical case for a
small kernel where 9 weights compete for the L2 budget, the two
normalizations behave very differently. Under L2 normalization the
outlier maps near $1$ in the sparse tail of the Beta codebook, where
centroid spacing is largest, and the per-coordinate error from rounding
...
$L = 2^{b-1} - 1$ and is reconstructed without rounding error, while
the non-outlier coordinates compress to small magnitudes that the
uniform grid handles coarsely but symmetrically. Large-fan-in linear
layers spread their energy across many coordinates and therefore favor
L2 normalization; depthwise kernels concentrate their energy in a few
coordinates and therefore favor absmax. The L2-versus-absmax tradeoff
is a separate axis from the Beta-concentration argument above, but the
two mechanisms reinforce each other on the same set of layers, so we
treat them jointly in Section~\ref{sec:experiments-boundary} and in
Appendix~\ref{app:boundary}.

The practical rule from this analysis is that SphereQuant should be preferred when
...
SphereQuant introduces no overhead at inference time beyond the one-time
...
SphereQuant is the composition of three well-understood ingredients. We normalize
