...
Table~\ref{tab:fanin-beta} makes the transition explicit. The
standard deviation of $\mathrm{Beta}(d/2, d/2)$ on $[-1, 1]$ scales as
$(d+1)^{-1/2}$, and the codebook advantage over a uniform grid on
$[-1, 1]$ is meaningful only when this standard deviation is small
enough that the centroids cluster near the origin. The third column
lists representative layers spanning the architectures in our study.

\begin{table}[h]
\centering
\caption{Beta$(d/2, d/2)$ standard deviation on $[-1, 1]$ as a function
of fan-in $d$, with representative layers from the architectures we
study. The codebook advantage requires the rotated coordinate
distribution to be sharply concentrated, which fails for
$d \lesssim 32$.}
\label{tab:fanin-beta}
\begin{tabular}{rll}
\hline
$d$  & Beta std on $[-1, 1]$ & Representative layer \\
\hline
2048 & 0.022 & TinyLlama / Phi-1.5 attention and MLP linear layers \\
768  & 0.036 & ViT-B/16 hidden \\
256  & 0.062 & ResNet-50 mid-stage convolutions \\
96   & 0.10  & MobileNet-V2 pointwise convolutions \\
32   & 0.17  & EfficientNet-B0 squeeze-excitation bottleneck \\
9    & 0.32  & MobileNet-V2 / EfficientNet-B0 $3{\times}3$ depthwise \\
\hline
\end{tabular}
\end{table}

EfficientNet-B0 in particular contains layers from three fan-in
regimes simultaneously: pointwise convolutions in the favorable
$d \geq 96$ regime, depthwise convolutions at $d \in \{9, 25\}$, and
squeeze-excitation bottlenecks at $d \in \{4, 8, 16, 32\}$ that
sit at or below the empirical boundary. The accuracy gap to QuaRot-RTN
therefore depends on which regime carries the dominant share of the
quantization error budget at the chosen bit width, which itself shifts
with bit width because the regimes saturate at different rates.
Appendix~\ref{app:boundary} decomposes this regime change in detail.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_rotation_distributions_vision.png}
\caption{Per-row L2-normalized weight coordinate distributions, before
and after SphereQuant's rotation, for three representative layers per
ImageNet model spanning the fan-in range. Blue is the pre-rotation
empirical histogram, orange is the post-rotation histogram, the red
curve is the predicted Beta$(d/2, d/2)$ density. Large-$d$ panels
(right columns of upper rows, all of ViT-B/16) show the post-rotation
histogram locking tightly onto Beta. Small-$d$ panels (leftmost
columns of MobileNet-V2 and EfficientNet-B0 at $d{=}9$) show the
boundary failure: Beta itself is broad and the post-rotation
distribution is nearly flat, so the matched codebook degenerates
toward uniform spacing and offers no advantage over QuaRot.}
\label{fig:rotation-vision}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_rotation_distributions_llm.png}
\caption{Same diagnostic for TinyLlama 1.1B and Phi-1.5 1.3B,
showing the attention $q$-projection and two MLP projections per
model. Every linear layer in both models sits in the
$d \in \{2048, 5632, 8192\}$ regime, the post-rotation
histograms lock onto a sharp Beta in every panel, and there are no
small-$d$ failure modes of the kind visible in
Figure~\ref{fig:rotation-vision}. The TinyLlama $q$-projection has a
pronounced near-zero spike before rotation that the rotation
redistributes into the Beta envelope, an instructive visualization of
what the rotation does to outlier-concentrated weight rows.}
\label{fig:rotation-llm}
\end{figure}

Figures~\ref{fig:rotation-vision} and \ref{fig:rotation-llm} make the
boundary visible at a glance. The narrowness of Beta$(d/2, d/2)$
relative to the codebook's $[-1, 1]$ support is the geometric content
of Table~\ref{tab:fanin-beta}: when the density occupies essentially
the central tenth of the codebook range (as for $d \geq 96$), the
Lloyd-Max levels cluster where probability mass actually lives and
provide a meaningful gain over a uniform grid; when the density
occupies more than half the codebook range (as for $d = 9$), the
Lloyd-Max levels are nearly evenly spaced and the matched codebook
collapses onto QuaRot's uniform alternative.
